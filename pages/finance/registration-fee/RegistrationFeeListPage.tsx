        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`ফি সংগ্রহের বিবরণ`} size="4xl">
            ...
            {details && (
                 <div className="space-y-4">
                    ...
                    <h4 className="text-lg font-semibold text-black border-b pb-2 mb-2 pt-3">মারহালা-ভিত্তিক ফি</h4>
                    <div className="max-h-48 overflow-y-auto pr-2">
                        <table className="min-w-full text-sm">
                            <thead className="bg-gray-100"><tr>
                                <th className="p-2 text-right text-xs font-medium text-black uppercase tracking-wider">মারহালা</th>
                                <th className="p-2 text-right text-xs font-medium text-black uppercase tracking-wider">নিয়মিত</th>
                                <th className="p-2 text-right text-xs font-medium text-black uppercase tracking-wider">অনিয়মিত</th>
                                <th className="p-2 text-right text-xs font-medium text-black uppercase tracking-wider">মোট ফি (৳)</th>
                            </tr></thead>
                            <tbody className="divide-y divide-gray-200">
                            {details.registration_details?.map((d: any, i: number) => (
                                <tr key={i}>
                                    <td className="p-2 text-right">{d.marhalaNameBn}</td>
                                    <td className="p-2 text-right">{d.regularStudents.toLocaleString('bn-BD')}</td>
                                    <td className="p-2 text-right">{d.irregularStudents.toLocaleString('bn-BD')}</td>
                                    <td className="p-2 text-right font-semibold text-emerald-700">{d.calculatedFee.toLocaleString('bn-BD')}</td>
                                </tr>
                            ))}
                            </tbody>
                        </table>
...
const RegistrationFeeListPage: React.FC = () => {
  ...
  const { data: collectionsData, isLoading, error, refetch } = useQuery<{
    ...
    queryKey: ['collectionsList', 'registration_fee', currentPage, itemsPerPage, debouncedSearchTerm],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_collections_list', {
        p_page: currentPage,
        p_limit: itemsPerPage,
        p_search_term: debouncedSearchTerm.trim() || null,
        p_collection_type: 'registration_fee'
      });
  ...
  useEffect(() => { if (error) { addToast(error.message || 'নিবন্ধন ফি তালিকা আনতে সমস্যা হয়েছে।', 'error'); } }, [error, addToast]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-semibold text-gray-800">নিবন্ধন ফি তালিকা</h2>
        <Button onClick={() => navigate('/finance/registration-fee/collect')} leftIcon={<PlusCircleIcon className="w-5 h-5"/>}>নতুন নিবন্ধন ফি</Button>
      </div>
      <Card>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200"><thead className="bg-gray-50"><tr>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  {collections.length === 0 && !isLoading && (<tr><td colSpan={8} className="px-6 py-10 text-center text-sm text-gray-500">কোনো নিবন্ধন ফি সংগ্রহের তথ্য পাওয়া যায়নি। <Button variant="ghost" onClick={() => refetch()}>পুনরায় চেষ্টা করুন</Button></td></tr>)}
                </tbody>
              </table>
            </div>
...
export default RegistrationFeeListPage;